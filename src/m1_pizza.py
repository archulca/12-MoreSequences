...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(160.0, 20.0),  # All numbers are approximate.
                rg.Point(145.0, 46.0),
                rg.Point(115.0, 46.0),
                rg.Point(100.0, 20.0),
                rg.Point(115.0, -6.0),
                rg.Point(145.0, -6.0)]
    circle = rg.Circle(rg.Point(130, 20), 30)
    answer = generate_points_on_circle(circle, 6)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    new_points = generate_points_on_circle(circle,number_of_points)
    for k in range (len(new_points)):
        new_points[k].attach_to(window)
        new_circle = rg.Circle(new_points[k],10)
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  6 slices, thin (thickness=4) black lines.'
    window = rg.RoseWindow(400, 400, title)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    new_points = generate_points_on_circle(circle,number_of_slices)
    for k in range (len(new_points)):
        line = rg.Line(circle.center,new_points[k])
...
def run_test_polygon():
    ...
    title = ('POLYGON test 3:  6 segments with thick red lines;')
    window = rg.RoseWindow(550, 400, title)
    circle = rg.Circle(rg.Point(100, 200), 50)
    circle.outline_thickness = 2
    polygon(window, circle, 6, 'purple', 5)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.fill_color = color
    circle.attach_to(window)
    new_points = generate_points_on_circle(circle,number_of_segments)
    for k in range (len(new_points)):
        line = rg.Line(new_points[k],new_points[k-1])
        line.attach_to(window)
        new_points[k].attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  51 lime purple lines on yellow circle,'
             + ' hops = 25.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(250, 300), 100)
    circle.fill_color = 'yellow'
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # -------------------------------------------------------------------------
    circle.attach_to(window)
    new_points = generate_points_on_circle(circle,number_of_lines)
    for k in range(len(new_points)):
        if k + hops_to_next_point % len(new_points) != 0 :
            a = 0 +((k + hops_to_next_point) % len(new_points))
        else:
            a = k + hops_to_next_point
        line = rg.Line(new_points[k],new_points[a])
        line.arrow = 'last'
        line.attach_to(window)
        new_points[k].attach_to(window)
    window.render()
